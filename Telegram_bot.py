...
def start_message(message):
    bot.send_message(
        message.chat.id, "Привет! Я могу показать тебе текущую погоду в любом городе.")
    bot.send_message(
        message.chat.id, "Просто введи /weather, а после - название города ;) ")
    user_requests = get_user_history(message.chat.username)
    if user_requests:
        bot.send_message(
            message.chat.id, "Еще у меня есть список твоих предыдущих запросов")
        bot.send_message(
            message.chat.id, "Если ты хочешь посмотреть историю своих запросов, нажми /history")
...
def click_show_history(message):
    ...
    if not user_requests:
        ...
        bot.send_message(
            message.chat.id, f"Топ 5 предыдущих запросов: {user_requests}")
        bot.send_message(
            message.chat.id, "Если вы хотите удалить всю историю своих запросов, нажмите /clear_history")
...
def send_weather(message):
    ...
    if not weather.correct_response:
        bot.send_message(
            message.chat.id, "Вы ввели неверный адрес, пожалуйста повторите попытку")
    else:
        bot.send_message(
            message.chat.id, f"""Сегодня в {weather.city_name} {int(weather.temperature)}°, скорость ветра {int(weather.wind_speed)} м/с, давление {weather.pressure}, погодные условия {weather.condition}, местное время {weather.datetime}""")
